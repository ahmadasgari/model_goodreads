...
from peewee import CharField, ForeignKeyField, DateField, \
    DateTimeField, MySQLDatabase, Model, SmallIntegerField, TextField

from playhouse.db_url import connect

database = MySQLDatabase('database_name', user='', password='',
                         host='127.0.0.1', port=3306)
# database = connect('mysql://ahmad:Safar128@@127.0.0.1:3306/goodreads')
...
class BookShelf(BaseModel):
    ...
    def change_to_read(self):
        read_shelf = self.user.shelves.where(Shelf.name ==
        Shelf.READ).first()
...
class BookTranslator(BaseModel):
    ...
    translator = ForeignKeyField(Translator, backref='translated_books')
